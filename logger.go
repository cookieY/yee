...
func (l *Logger) logWrite(msgText string, level int) error {
	...
	if level != Trace {
		...
		msgText = fmt.Sprintf("%s [%s] %s %s: %s", YeeVersion, mappingLevel[level], time.Now().Format(timeFormat), src, msgText)
	} else {
		msgText = fmt.Sprintf("%s [%s] %s %s", YeeVersion, mappingLevel[level], time.Now().Format(timeFormat), msgText)
	}
